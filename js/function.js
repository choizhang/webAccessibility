...
$.fn.tab = function(param){
	var timeid,intervalid,content,plistli,nowli,nowdiv,preli,prediv,tab, plist, listafter, pcontent, action, len, nowindex,time,type,leftarr,rightarr,myScroll,flag_animate,phone_tipbar,access_ctrl;
	ptab = '.' + param.tab;
	plist = '.' + param.plist;
		...
		$(document).keydown(function(e){
	    	...
	    	switch (varkey){
	    		case 39:  //向右键
					...
					$('.' + listafter).children('a').trigger('focus'); //只有focus才能读出选项卡里面的菜单，这个值是需要动态获取的
					break;
				case 37:  //向左键
		    		switchTab(nowindex, nowindex - 1);
					$('.' + listafter).children('a').trigger('focus');
					break;
				...
				case 33:  //page up，这里采用alt+pageup到上一个选项卡
					if(access_ctrl){
		        		target.prev().children("a").focus();
		        	}
					break;
				case 34: //page down，这里采用alt+pagedown到下一个选项卡
					if(access_ctrl){
		        		target.next().children("a").focus();
		        	}
					break;
	    	}
	    });
